...
import { minesMultiplier, shuffle } from '@/lib/gameUtils';
...
function buildGrid(bombs: number): TileState[] {
  const grid = Array(25).fill('safe') as TileState[];
  const indices = shuffle([...Array(25).keys()]).slice(0, bombs);
  indices.forEach((i) => { grid[i] = 'bomb'; });
  return grid.map(() => 'hidden') as TileState[];
}

function placeBombs(bombs: number): TileState[] {
  const grid = Array(25).fill('safe') as TileState[];
  const indices = shuffle([...Array(25).keys()]).slice(0, bombs);
  indices.forEach((i) => { grid[i] = 'bomb'; });
  return grid;
...
export default function MinesPage() {
  ...
  const [truth, setTruth]   = useState<TileState[]>(Array(25).fill('hidden'));
  ...
  function startGame() {
    ...
    const grid = placeBombs(bombCount);
    setTruth(grid);
  ...
  function pickTile(idx: number) {
    ...
    if (actual === 'bomb') {
      // Reveal all bombs
      const full = truth.map((t, i) => (newRev[i] !== 'hidden' ? newRev[i] : t)) as TileState[];
      setRevealed(full);
      setWon(false);
      setFinalMult(0);
      setPhase('over');
  ...
  function cashOut() {
    ...
    setPhase('over');
    // Reveal all bombs
    const full = truth.map((t, i) => (revealed[i] !== 'hidden' ? revealed[i] : t)) as TileState[];
    setRevealed(full);
  ...
  const controls = (
    <Card className="p-4 space-y-4">
      ...
      <div>
        ...
        <input
          type="range" min={1} max={24} value={bombCount}
