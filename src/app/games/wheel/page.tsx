...
import { WHEEL_SEGMENTS, wheelResultFromFloat } from '@/lib/gameUtils';
...
export default function WheelPage() {
  ...
  const pf = useProvablyFair('wheel');
  ...
  async function spin() {
    ...
    try {
      ...
      const segIdx = wheelResultFromFloat(floats[0]);
      const segment = SEGMENTS_WITH_ANGLES[segIdx];

      // Animate to the derived segment
    ...
    <Card className="p-4">
      <div className="flex flex-col items-center">
        ...
        <div className="relative" style={{ width: 260, height: 260 }}>
          <svg width={260} height={260} viewBox="0 0 260 260" aria-label="Wheel of Fortune"
            style={{
            ...
            {SEGMENTS_WITH_ANGLES.map((seg, i) => (
              <g key={i}>
                <path d={segmentPath(seg.startDeg, seg.endDeg)} fill={seg.color} stroke="#0a0e1a" strokeWidth={2} />
                <text
        ...
        {pf.lastReveal && (
          <details className="mt-4 w-full text-xs text-gray-500 bg-white/5 rounded-xl p-3">
